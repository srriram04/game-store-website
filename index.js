...
/* Mobile toggle hints (visible only on mobile via CSS) */
...
// Section scroll down — dynamic offset based on viewport
document.querySelectorAll('a[href^="#"]').forEach(link => {
    link.addEventListener("click", function (e) {
        ...
        if (target) {
            e.preventDefault();
            // Mobile navbar is 60px, desktop is 100px
            const offset = window.innerWidth <= 768 ? 68 : 110;
...
if (hamburger && sidebar && overlay) {
    ...
    // Close when tapping the dim overlay (outside sidebar)
    overlay.addEventListener("click", closeSidebar);

    // Close automatically after tapping a nav link
